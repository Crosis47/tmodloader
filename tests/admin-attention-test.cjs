  ...
  try {
    ...
    await page.route('http://recovery.test/**', async route => {
      ...
      if (url.pathname === '/api/mod-configs') {
        ...
        data = url.searchParams.has('name') || route.request().method() === 'POST' ? {name, content: modContent, format: name.endsWith('.yaml') ? 'YAML' : 'JSON', revision: 'file-revision'} : {files: ['Example_Server.json', 'Custom.yaml']};
      }
    ...
    await page.locator('#mod-config-file').selectOption('Custom.yaml');
    await page.locator('#mod-config-open').click();
    ...
    await page.locator('#mod-config-save').click();
    await page.locator('#mod-config-status').filter({hasText: 'Saved Custom.yaml'}).waitFor();
