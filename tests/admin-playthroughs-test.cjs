...
(async () => {
  ...
  try {
    ...
    await page.route('http://worlds.test/**', async route => {
      ...
      if (url.pathname === '/api/playthroughs') {
        if (route.request().method() === 'POST') {
          const body = route.request().postDataJSON();
          if (body.action === 'save') playthroughs.push({id: String(++sequence), name: body.name, settings: {...(body.source === 'running' ? running : staged), TMOD_WORLDNAME: body.world}});
          if (body.action === 'rename') playthroughs.find(p => p.id === body.id).name = body.name;
          if (body.action === 'delete') { assert.equal(body.confirm, true); playthroughs = playthroughs.filter(p => p.id !== body.id); }
          if (body.action === 'stage') { staged = {...staged, ...playthroughs.find(p => p.id === body.id).settings}; pending = true; }
        }
        data = {playthroughs, revision: 'revision', catalog_revision: 'catalog', running, staged, worlds: [{name:'Original',can_select:true}], editable, pending, busy: false};
      }
    ...
    await page.getByRole('button', {name: 'Playthroughs', exact: true}).click();
    await page.locator('#playthrough-name').fill('Vanilla'); await page.locator('#playthrough-save').click();
    await page.getByRole('heading', {name: 'Vanilla', exact: true}).waitFor();
    await page.getByRole('button', {name: 'Load playthrough', exact: true}).click();
    ...
    await page.locator('#playthroughs-apply').click(); await page.locator('#confirm-go').click();
    ...
    await page.evaluate(() => refreshPlaythroughs(true));
    ...
    assert.equal(playthroughs.length, 1);
    editable = false; await page.locator('#playthroughs-refresh').click();
    await page.locator('#playthroughs-status').filter({hasText: 'Enable web-managed'}).waitFor();
    assert.equal(await page.getByRole('button', {name: 'Load playthrough', exact: true}).isDisabled(), true);
    await page.setViewportSize({width: 390, height: 844});
    assert.equal(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth), true);
    assert.deepEqual(errors, []);
    console.log('Playthroughs save, load, cancel, apply, rename, delete cancellation and read-only/mobile checks passed.');
