...
(async () => {
  ...
  try {
    ...
    await page.locator('#recovery-preview').click();
    await page.locator('#recovery-preview-detail').getByText('Worlds: Recovery.wld', {exact: true}).waitFor();
    await page.locator('#recovery-restore').click(); await page.locator('#confirm-cancel').click(); assert.equal(restores, 0);
    await page.locator('#recovery-restore').click(); await page.locator('#confirm-go').click();
